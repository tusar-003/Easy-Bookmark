// Firefox-compatible script for Easy Bookmarks
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function loadMostVisitedSites() {
    browserAPI.topSites.get().then((sites) => {
      ...
      browserAPI.storage.local.get(["mostVisitedCustomNames"]).then((result) => {
  ...
  function removeMostVisitedSite(url) {
    // Firefox's topSites API doesn't provide a direct way to remove sites,
    // but we can use browser.history.deleteUrl to achieve a similar effect
    browserAPI.history.deleteUrl({ url: url }).then(() => {
      // Remove any custom name for this URL
      browserAPI.storage.local.get(["mostVisitedCustomNames"]).then((result) => {
        const customNames = result.mostVisitedCustomNames || {}
        if (customNames[url]) {
          delete customNames[url]
          browserAPI.storage.local.set({ mostVisitedCustomNames: customNames })
        }
  ...
  function editMostVisitedSite(url, newTitle) {
    browserAPI.storage.local.get(["mostVisitedCustomNames"]).then((result) => {
      ...
      browserAPI.storage.local.set({ mostVisitedCustomNames: customNames }).then(() => {
  ...
  function loadBookmarks() {
    browserAPI.bookmarks.getTree().then(async (bookmarkTreeNodes) => {
  ...
  // This is the most reliable single source for Firefox
  ...
  // Load favicon with fallback to default
  ...
  function deleteGroup(groupId, deleteBookmarks = false) {
    ...
    if (deleteBookmarks) {
      ...
      group.bookmarkIds.forEach(bookmarkId => {
        browserAPI.bookmarks.remove(bookmarkId).catch((error) => {
  ...
  function saveGroups() {
    browserAPI.storage.local.set({ bookmarkGroups: bookmarkGroups })
  }

  // Load groups from storage
  function loadGroups() {
    return browserAPI.storage.local.get(["bookmarkGroups", "groupOrder"]).then((result) => {
  ...
  function deleteBookmark(bookmarkId) {
    browserAPI.bookmarks.remove(bookmarkId).then(() => {
  ...
  function deleteSelectedBookmarks() {
    ...
    const deletePromises = bookmarksToDelete.map((bookmarkId) => {
      return browserAPI.bookmarks.remove(bookmarkId).then(() => {
        deletedCount++
      }).catch((error) => {
        console.error("Error deleting bookmark:", error)
      })
  ...
  addBookmarkForm.addEventListener("submit", (e) => {
    ...
    // Add the bookmark to Firefox's bookmarks
